...
{else}							{*** PREPARE (FOR ALL) ***}
	...
	{if $v[0] == 'CC' or $v[0] == 'RR' or $v[0] == 'Y' or $v[0] == 'W' or $v[0] == 'XX' or $v[0] == 'L'}
		<div class="form_block_el cfix">
			...
			{else}
				...
				{elseif $v[0] == 'W'}
					<textarea{$n}{$hint}cols="60"{if $v.size > 0} rows="{$v.size}"{/if} wrap="off" {*class="text{$class}{$error_class}"*} {if $f ne "pDescr"}class="ckeditor"{/if}>{$cv}</textarea>
				{elseif $v[0] == 'XX'}
	...
	{else}
		<label for="">{$l}</label>
        <div class="form_block_el_right {if $v[0] == 'C'}checkbox{/if}">
			{$error_text}
			{if $v[0] == 'X'}
				<span class="value{$class}">
